import { generateWithRetry } from '@/infrastructure/ai/client'
import { getPhaseConfig } from '@/infrastructure/ai/router'
import { CreativeBriefSchema, type CreativeBrief, type TaskSpec } from '@/lib/schemas'
import { SYSTEM_PROMPTS, buildCreativeBriefPrompt } from '@/core/prompts/templates'
import { getCopyTypeRules } from '@/core/copy-type-rules'

export async function generateCreativeBrief(
  taskSpec: TaskSpec
): Promise<CreativeBrief> {
  const config = getPhaseConfig('creative_brief')
  
  // Get copy type constraints to enforce single_job appropriately
  const copyTypeRules = getCopyTypeRules(taskSpec.copy_type)
  const constraints = {
    maxBeats: copyTypeRules.maxBeats,
    maxWords: copyTypeRules.maxTotalWords,
    targetWords: copyTypeRules.targetWords,
  }
  
  const result = await generateWithRetry({
    config,
    schema: CreativeBriefSchema,
    system: SYSTEM_PROMPTS.creative_brief,
    prompt: buildCreativeBriefPrompt(JSON.stringify(taskSpec, null, 2), constraints),
  })
  
  return result
}

